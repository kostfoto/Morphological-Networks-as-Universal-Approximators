...
// __global__ void maxplus_conv2d_kernel(
...
//                         int input_idx = ((b * in_channels + ic) * in_height + ii) * in_width + jj;
//                         int weight_idx = ((oc * in_channels + ic) * kernel_size + ki) * kernel_size + kj;
//                         max_val = fmaxf(max_val, input[input_idx] + weight[weight_idx]);
//                     }
//                 }
//             }
//         }
//         if (bias != nullptr) {
//             max_val = fmaxf(max_val, bias[oc]);
//         }
//         int output_idx = ((b * out_channels + oc) * out_height + i) * out_width + j;
//         output[output_idx] = max_val;
//     }
// }

// torch::Tensor maxplus_conv2d_cuda(
...
//     int out_height = (in_height + 2 * padding_val - kernel_size) / stride_val + 1;
//     int out_width = (in_width + 2 * padding_val - kernel_size) / stride_val + 1;

//     torch::Tensor output = torch::full({batch, out_channels, out_height, out_width},
//                                        -1e9, input.options());

//     dim3 blocks(batch, out_channels);
//     dim3 threads(out_height, out_width);

//     maxplus_conv2d_kernel<<<blocks, threads>>>(
//         input.data_ptr<float>(), weight.data_ptr<float>(),
//         bias.defined() ? bias.data_ptr<float>() : nullptr,
//         output.data_ptr<float>(), batch, in_channels, out_channels,
//         in_height, in_width, out_height, out_width, kernel_size, stride_val, padding_val);

//     return output;
// }

// // Expose function for binding in another file
// // TORCH_LIBRARY_FRAGMENT(maxplus_conv2d, m) {
// //     m.def("maxplus_conv2d_cuda", &maxplus_conv2d_cuda);
// // }

// PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
//     // m.def("maxplus_conv2d_cpu", &maxplus_conv2d_cpu, "Max-Plus Conv2D (CPU)");
//     m.def("maxplus_conv2d_cuda", &maxplus_conv2d_cuda, "Max-Plus Conv2D (CUDA)");
// }
...
//     const float* __restrict__ input, const float* __restrict__ weight, 
//     const float* __restrict__ bias, float* __restrict__ output, int* __restrict__ argmax_idx,
//     int batch, int in_channels, int out_channels,
//     int in_height, int in_width, int out_height, int out_width,
//     int kernel_size, int stride, int padding) {
...
//                         if (val > max_val) {
//                             max_val = val;
//                             max_idx = input_idx;  // Store the index of the max value
//                         }
//                     }
//                 }
//             }
...
//             if (bias[oc] > max_val) {
//                 max_val = bias[oc];
//                 max_idx = -1;  // Bias dominates, no input contributes
//             }
//         }
...
//     const int* __restrict__ argmax_idx, const float* __restrict__ grad_output,
...
//         if (max_idx >= 0) {
//             atomicAdd(&grad_input[max_idx], grad_output[output_idx]);
//         } else {
//             atomicAdd(&grad_bias[oc], grad_output[output_idx]);
//         }
//     }
// }
...
//     torch::Tensor output = torch::full({batch, out_channels, out_height, out_width},
//                                        -1e9, input.options());
//     torch::Tensor argmax_idx = torch::full_like(output, -1, input.options().dtype(torch::kInt32));
...
//         input.data_ptr<float>(), weight.data_ptr<float>(),
//         bias.defined() ? bias.data_ptr<float>() : nullptr,
//         output.data_ptr<float>(), argmax_idx.data_ptr<int>(),
//         batch, in_channels, out_channels,
//         in_height, in_width, out_height, out_width, kernel_size, stride_val, padding_val);

//     return {output, argmax_idx};
// }

// std::vector<at::Tensor> maxplus_conv2d_backward(
//     torch::Tensor grad_output, torch::Tensor argmax_idx,
//     int batch, int in_channels, int out_channels,
//     int in_height, int in_width, int out_height, int out_width,
//     int kernel_size, int stride, int padding) {

//     torch::Tensor grad_input = torch::zeros({batch, in_channels, in_height, in_width}, grad_output.options());
//     torch::Tensor grad_weight = torch::zeros({out_channels, in_channels, kernel_size, kernel_size}, grad_output.options());
//     torch::Tensor grad_bias = torch::zeros({out_channels}, grad_output.options());

//     dim3 blocks(batch, out_channels);
//     dim3 threads(out_height, out_width);

//     maxplus_conv2d_backward_kernel<<<blocks, threads>>>(
//         argmax_idx.data_ptr<int>(), grad_output.data_ptr<float>(),
...
//     std::vector<at::Tensor> grads = {grad_input, grad_weight, grad_bias};
//     return grads;
...
std::vector<torch::Tensor> maxplus_conv2d_forward(
    ...
    // dim3 blocks((H_out + BLOCK - 1) / BLOCK, C_out, B);
    dim3 threads(BLOCK, BLOCK);
